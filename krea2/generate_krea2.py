"""
Generate all 9 images with Krea 2 Turbo — 8 steps, 1280×720, FP8 model.
"""
...
WORKFLOW = {
    ...
    "4": {"class_type": "CLIPTextEncode", "inputs": {"text": "ugly, blurry, distorted, low quality, bad anatomy", "clip": ["2", 0]}},
    "5": {"class_type": "ConditioningZeroOut", "inputs": {"conditioning": ["4", 0]}},
    "6": {"class_type": "VAELoader", "inputs": {"vae_name": "qwen_image_vae.safetensors"}},
    "7": {"class_type": "EmptyLatentImage", "inputs": {"width": 1280, "height": 720, "batch_size": 1}},
    "8": {"class_type": "KSampler", "inputs": {"seed": 42, "steps": 8, "cfg": 3.5, "sampler_name": "euler", "scheduler": "beta", "denoise": 1.0, "model": ["1", 0], "positive": ["3", 0], "negative": ["5", 0], "latent_image": ["7", 0]}},
    "9": {"class_type": "VAEDecode", "inputs": {"samples": ["8", 0], "vae": ["6", 0]}},
    "10": {"class_type": "SaveImage", "inputs": {"filename_prefix": "krea2", "images": ["9", 0]}}
}
...
print("=" * 60)
print("🎨 Krea 2 Turbo — Generating 9 images at 1280×720")
print("   Model: krea2_turbo_fp8_scaled.safetensors (FP8)")
print("   Settings: 8 steps, CFG 3.5, euler/beta")
...
for i, img in enumerate(IMAGES):
    ...
    if os.path.exists(outpath):
        print(f"\n[{i+1}/9] ⏭ {fname} — exists, skipping")
        continue
    
    print(f"\n[{i+1}/9] 🎯 {fname}")
    print(f"   {img['prompt'][:120]}...")
    
    # Submit
    seed = random.randint(0, 2**63 - 1)
    wf = json.loads(json.dumps(WORKFLOW))
    wf["3"]["inputs"]["text"] = img["prompt"]
    wf["8"]["inputs"]["seed"] = seed
    wf["10"]["inputs"]["filename_prefix"] = name
    
    payload = {"prompt": wf, "client_id": "hermes-krea2-batch"}
            ...
            if 'outputs' in entry:
                for nid, outs in entry['outputs'].items():
                    for oimg in outs.get('images', []):
                        ...
                        print(f"   ✅ {fname} ({len(imgdata)/1024:.0f} KB) in {elapsed}s")
                break
...
for f in sorted(os.listdir(OUTDIR)):
    if f.endswith('.png'):
        s = os.path.getsize(os.path.join(OUTDIR, f)) / 1024
        print(f"   📁 {f} ({s:.0f} KB)")
